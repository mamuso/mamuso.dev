...
- **Proof.** Snapshot and screenshot the year archive (not a single note). Save `.agents/skills/verify-mamuso-dev/artifacts/notes-archive/years.aria.txt` and `.agents/skills/verify-mamuso-dev/artifacts/notes-archive/years.png`. Both show `Journal`, a year heading, and `Expand all notes ↓`.
