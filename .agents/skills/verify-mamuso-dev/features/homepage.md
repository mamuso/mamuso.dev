...
- **Proof.** Save an ARIA snapshot and screenshot. Run a snapshot to `.agents/skills/verify-mamuso-dev/artifacts/homepage/home.aria.txt` and a screenshot to `.agents/skills/verify-mamuso-dev/artifacts/homepage/home.png`. Both identify `mamuso` and `Manuel Muñoz Solera`.
