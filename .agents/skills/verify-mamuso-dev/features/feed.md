...
- **Fetch.** Run `.agents/skills/verify-mamuso-dev/bin/control-mamuso curl --path /feed.xml --out .agents/skills/verify-mamuso-dev/artifacts/feed/feed.xml`. Status is `200`. `content-type` includes `xml`.
...
- **Proof.** Keep the saved `feed.xml` body. Also save curl stdout as `.agents/skills/verify-mamuso-dev/artifacts/feed/headers.txt`.
