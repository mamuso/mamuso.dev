...
- **Legacy redirect (HTTP).** Run `.agents/skills/verify-mamuso-dev/bin/control-mamuso curl --path /post/2023-08-21-cozy`. Status is `308` (or `301`) and `location` ends with `/note/2023-08-21-cozy`.
...
- **Proof.** Snapshot and screenshot the article. Save `.agents/skills/verify-mamuso-dev/artifacts/note/cozy.aria.txt` and `.agents/skills/verify-mamuso-dev/artifacts/note/cozy.png`. Both show header `mamuso` and heading `Cozy`. Save the redirect headers as `.agents/skills/verify-mamuso-dev/artifacts/note/legacy-redirect.txt` (the curl stdout).
