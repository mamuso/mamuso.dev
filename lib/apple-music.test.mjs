...
test('fixed upstream, no redirects or persistent fetch cache; concurrent requests deduplicate', async () => {
  ...
  const read = createRecentTrackReader({
    ...
    request: async (url, options) => {
      calls++
      assert.equal(url, 'https://api.music.apple.com/v1/me/recent/played/tracks?limit=1&types=songs,library-songs&include[library-songs]=catalog')
...
test('a fresh library play replaces the cached catalog song when the cache expires', async () => {
  let time = 0
  const libraryResponse = { data: [{ type: 'library-songs', id: 'i.library', attributes: {
    name: 'New library song', artistName: 'Library artist',
  } }] }
  const read = createRecentTrackReader({
    credentials: () => ({ developerToken: 'FAKE', userToken: 'FAKE' }),
    now: () => time,
    request: async url => {
      if (new URL(url).pathname.endsWith('/catalog')) return Response.json({ data: [] })
      const types = new URL(url).searchParams.get('types')?.split(',') ?? []
      assert.ok(types.includes('songs'))
      assert.ok(types.includes('library-songs'))
      return Response.json(time === 0 ? response : libraryResponse)
    },
  })
  assert.equal((await read()).track.name, 'Song')
  time = 59_999
  assert.equal((await read()).track.name, 'Song')
  time = 60_000
  assert.deepEqual((await read()).track, {
    name: 'New library song', artist: 'Library artist', artwork: null, bgColor: null, url: null,
  })
})

test('library history uses its exact catalog metadata without changing the played song', async () => {
  for (const included of [true, false]) {
    let calls = 0
    let signal
    const catalog = structuredClone(response)
    catalog.data[0].attributes.artwork.bgColor = 'aabbcc'
    const library = { data: [{ type: 'library-songs', id: 'i.library', attributes: {
      name: 'Library title', artistName: 'Library artist',
      artwork: { url: 'https://is1-ssl.mzstatic.com/library/{w}x{h}.jpg' },
    }, relationships: included ? { catalog } : undefined }] }
    const read = createRecentTrackReader({
      credentials: () => ({ developerToken: 'FAKE', userToken: 'FAKE' }),
      now: () => 0,
      request: async (url, options) => {
        ...
        if (calls === 1) {
          ...
          assert.equal(new URL(url).searchParams.get('include[library-songs]'), 'catalog')
          return Response.json(library)
        }
        assert.equal(url, 'https://api.music.apple.com/v1/me/library/songs/i.library/catalog')
    ...
    assert.deepEqual(results[0].track, {
      name: 'Library title', artist: 'Library artist', bgColor: '#aabbcc',
      artwork: 'https://is1-ssl.mzstatic.com/library/160x160.jpg',
      url: 'https://music.apple.com/us/album/example/456?i=123',
    })
    assert.equal(results[0].refreshAfterMs, 60_000)
    await read()
    assert.equal(calls, included ? 1 : 2)
  }
})

test('missing or failed catalog metadata preserves the latest library song', async () => {
  for (const catalogRequest of [
    ...
    const read = createRecentTrackReader({
      credentials: () => ({ developerToken: 'FAKE', userToken: 'FAKE' }),
      request: async () => ++calls === 1 ? Response.json({ data: [{
        type: 'library-songs', id: 'i.library',
        attributes: { name: 'Latest song', artistName: 'Artist' },
      }] }) : catalogRequest(),
    })
    const result = await read()
    assert.equal(result.status, 'ready')
    assert.equal(result.track.name, 'Latest song')
    assert.equal(result.track.bgColor, null)
    assert.equal(calls, 2)
  }
})

test('catalog relationship URLs and invalid library identifiers cannot redirect authenticated requests', async () => {
  let calls = 0
  const read = createRecentTrackReader({
    credentials: () => ({ developerToken: 'FAKE', userToken: 'FAKE' }),
    request: async () => {
      calls++
      return Response.json({ data: [{ type: 'library-songs', id: '../private?token=secret',
        attributes: { name: 'Song', artistName: 'Artist' },
        relationships: { catalog: { href: 'https://evil.test/', data: [] } },
      }] })
    },
  })
