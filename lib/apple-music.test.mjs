...
test('transient outages retain the last public track for at most fifteen minutes and retry sooner', async () => {
  let time = 0
  let failure = false
  let calls = 0
  const read = createRecentTrackReader({
    ...
    request: async () => {
      calls++
      return failure ? new Response(null, { status: 503 }) : Response.json(response)
    },
  })
...
test('revoked authorization and empty history clear the cached track', async () => {
  for (const result of [new Response(null, { status: 401 }), new Response(null, { status: 403 }), Response.json({ data: [] })]) {
    let time = 0
    const read = createRecentTrackReader({
      credentials: () => ({ developerToken: 'FAKE', userToken: 'FAKE' }),
      now: () => time,
      request: async () => time === 0 ? Response.json(response) : result,
    })
