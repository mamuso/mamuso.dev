        ...
        {photoPosts.map((post) => (
          <div key={post.slug}>
            <Link href={`/note/${post.slug}`}>
              <Image src={`/assets/feed/gallery-${post.basename}`} sizes="(min-width: 1040px) 874px, (min-width: 900px) 807px, calc(94.31vw - 23px)" width={post.width / 4} height={post.height / 4} alt={post.title} />
            </Link>
          </div>
        ))}
      </div>
