...
export function useCartridgeBlow(isOpen: boolean, settled: RefObject<boolean>, stickerBusy: RefObject<boolean>) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('touchend', current.resumeAudio, { capture: true, passive: true })
    ...
    return () => {
      ...
      window.removeEventListener('touchend', current.resumeAudio, true)
  ...
  return {
    ...
    cancelForNavigation() {
      controller.current?.returnToOpen(performance.now())
      pointer.current = null
    },
    consumeClick() {
      const current = controller.current
