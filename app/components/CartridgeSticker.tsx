...
export default function CartridgeSticker({
  ...
  // The placement is cached per appliedRef, so this stays stable across
  // Strict Mode double renders and responsive canvas remounts. Only the ref's
  // identity is used as the cache key; its value is never read during render.
  // eslint-disable-next-line react-hooks/refs
  const placement = useMemo(() => stickerPlacement(appliedRef), [appliedRef])
  const { geometry, seated, width, rotation } = useMemo(
    () => createStickerSurface(scene, center, placement),
    [scene, center, placement]
  )
  const rotationRef = useRef(rotation)
  const motionRef = useRef(placement.motion ?? NEUTRAL_MOTION)
  useLayoutEffect(() => {
    rotationRef.current = rotation
    motionRef.current = placement.motion ?? NEUTRAL_MOTION
    invalidate()
  }, [rotation, placement, invalidate])
