  ...
  return <>
    ...
    {caption && <group ref={captionAnchor}>
      <Html center style={{ pointerEvents: 'none' }}>
        <div ref={captionElement} {...stylex.props(styles.caption(CAPTION_FONT_SIZE, CAPTION_LINE_HEIGHT))}>
          {caption.company}
          <span {...stylex.props(styles.period)}>{caption.period}</span>
        </div>
      </Html>
    </group>}
  </>
...
const styles = stylex.create({
  // The frame loop writes opacity and transform inline; these are the resting values.
  // StyleX cannot evaluate imports from non-.stylex modules at compile time,
  // so the shared caption metrics arrive as runtime values.
  caption: (fontSize: number, lineHeight: number) => ({
    opacity: 0, textAlign: 'center', whiteSpace: 'nowrap', fontSize, fontWeight: type.weightMedium, lineHeight,
  }),
  period: { display: 'block', opacity: 0.65, fontVariantNumeric: 'tabular-nums' },
})
