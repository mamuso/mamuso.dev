...
async function contentSelectionContracts() {
  const summary = await getPostBySlug('example', ['title', 'slug'])
  ...
  const photos = await getPhotoPosts(['width', 'iso'])
  ...
  const note = (await getNotePosts(['basename', 'width']))[0]
  ...
  const selected = await getPostBySlug('example', ['category', 'width'])
