...
for (const scenario of ['switch', 'cancel', 'scroll'] as const) {
  test(`cartridge touch gestures: ${scenario}`, async ({ page, isMobile }) => {
    // Each scenario gets a fresh page and budget on CI's software GPU.
    ...
    if (scenario === 'switch') {
      ...
      await waitForCartridgeLayout(page, controls.nth(2))
      await drag(2, 3, -100)
      if (isMobile) await expect.poll(() => page.evaluate(() => scrollY)).toBeGreaterThan(0)
      await expect(controls.nth(2)).toHaveAttribute('aria-expanded', 'true')
    }
